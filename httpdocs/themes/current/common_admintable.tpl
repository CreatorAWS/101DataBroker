...
<table{foreach name=table_attr_index from=$table.attrs item=attr key=attr_name} {$attr_name}="{$attr}"{/foreach} id="admintable{$postfix}" class="table table-editable no-margin mb-sm admintable{$table.class}">
	{if $table.hideheader neq 1}{strip}
	<thead>
		<tr class="admintable_header" role="row" id="admintable_header{$postfix}">
		...
		<th{if $column.hint neq ""} title="{$column.hint}"{elseif $column.caption neq ""} title="{$column.caption}"{/if}{if $column.width neq ""} width="{$column.width}"{/if}{if $column.align neq ""} align="{$column.align}"{/if}{if $column.headercolspan neq ""} colspan="{$column.headercolspan}"{/if}>
...
<tbody>
	...
	{strip}
	<tr class="admintable_row{$table.rowparams[table_row_index].class}"{if $table.rows[table_row_index].$default_column.url neq ""} onDblClick="admintable_goto{$postfix}('{$table.rows[table_row_index].$default_column.url}')"{/if}{if $table.rowparams[table_row_index].style neq ""} style="{$table.rowparams[table_row_index].style}"{/if}>
		{foreach name=table_column_index from=$table.columns item=column key=column_name}
