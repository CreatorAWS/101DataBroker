				...
				{foreach name=form_field_index from=$form.fields item=field key=field_name}
					{if $field.tab eq $smarty.section.tabsectionindex.index}
						{if $field.type neq "hidden" and  $field.type neq "separator" and $field.mergecolumns neq 1 and $field.hidedefinition neq 1}
							<tr  class="form-group{if $field.rowclassname neq ""} {$field.rowclassname}{/if}" id="admintablerow-{if $field.id neq ""}{$field.id}{else}{$form.prefix}{$field.name}{/if}">
							...
							<tr class="form-group{if $field.rowclassname neq ""} {$field.rowclassname}{/if}" id="admintablerow-{if $field.id neq ""}{$field.id}{else}{$form.prefix}{$field.name}{/if}">
							<td colspan="{if $form.tooltip_present}3{else}2{/if}">{$field.caption}
						{/if}
						...
						{elseif $field.type eq "text" or $field.type eq "password" or $field.type eq "hidden"}
							<input class="form-control" type="{$field.type}" name="{$form.prefix}{$field.name}" value="{$form.data.$field_db}" id="{if $field.id neq ""}{$field.id}{else}{$form.prefix}{$field.name}{/if}"{foreach name=form_field_attr_index from=$field.attrs item=attrval key=attrname} {$attrname}="{$attrval}"{/foreach} />
						...
						{elseif $field.type eq "textarea"}
							<textarea class="form-control" name="{$form.prefix}{$field.name}" cols="30" rows="5" name="1" id="{if $field.id neq ""}{$field.id}{else}{$form.prefix}{$field.name}{/if}"{foreach name=form_field_attr_index from=$field.attrs item=attrval key=attrname} {$attrname}="{$attrval}"{/foreach}>{$form.data.$field_db}</textarea>
